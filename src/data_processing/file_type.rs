...
pub trait DataFile<TVar: VariableDescriptor, TData: DataType, TMetadata: Metadata> {
    ...
    fn open(path: &Path, metadata: TMetadata) -> Result<Self, String> where Self: Sized;
...
impl<T: DataType + netcdf::Numeric> DataFile<String, T, CdfMetadata> for Nc<T> {
    fn extension() -> &'static OsStr {
        OsStr::new("nc")
    }

    fn open(path: &Path, metadata: CdfMetadata) -> Result<Self, String> where Self: Sized {
...
impl<T: DataType + netcdf::Numeric> DataFile<String, T, CdfMetadata> for Nc4<T> {
    fn extension() -> &'static OsStr {
        OsStr::new("nc4")
    }
...
impl<T: DataType + netcdf::Numeric> CdfReadableData<T> {
...
impl<T: DataType + netcdf::Numeric> CdfReadableData<T> {
    ...
    fn read_2d_variable(&self, v: &netcdf::Variable) -> Data2dStatistics<T> {
        ...
        for row in 0..height {
            for column in 0..width {
                ...
                if let Some((val, new_min, new_max)) =
                        Self::stat_cell(v, &indices, min, max) {
    ...
    fn stat_cell(
        ...
        if let Ok(val) = v.value::<T>(Some(indices)) {
