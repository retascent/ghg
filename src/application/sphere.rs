...
pub fn generate_sphere_with_position(subdivisions: u32,
    ...
    cube_normals().iter()
        .map(|n| generate_face::<QuadSubface>(
...
fn generate_face<T: SubfaceGenerator + Clone >(subface_generator: T,
                                               normal: &Vec3,
                                               subdivisions: u32,
                                               points_per_subdivision: u32,
                                               sphere_center: Vec3) -> Vec<BasicMesh> {
    ...
    for y in 0..subdivisions {
        for x in 0..subdivisions {
            ...
            meshes.push(generate_subface(subface_generator.clone(),
                                         normal,
...
fn generate_subface<T: SubfaceGenerator>(subface_generator: T,
                                         face_normal: &Vec3,
                                         points_per_side: u32,
                                         subdivision_start: Vec2,
                                         subdivision_side_length: f32,
                                         sphere_center: Vec3) -> BasicMesh {
    ...
    for y in 0..points_per_side {
        for x in 0..points_per_side {
            ...
            let point: Vec3 = (face_normal + axis_a * (2.0 * t.x - 1.0) + axis_b * (2.0 * t.y - 1.0)).into();

            let sphere_point = point_on_cube_to_point_on_sphere(point) + sphere_center;
            let normal: Vec3 = sphere_point.normalize();

            subface_generator.add_for_point(&mut mesh, x, y,
                                            face_normal,
                                            &normal, &face_color,
                                            points_per_side, &sphere_point);
        }
    }
...
impl SubfaceGenerator for QuadSubface {
    fn add_for_point(&self,
        ...
        mesh.push_vertex(Vertex::from_vecs(sphere_point.clone(),
                                           normal.clone(),
                                           face_color.clone()));
