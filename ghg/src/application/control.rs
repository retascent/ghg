...
impl Controller {
	pub fn new(
		...
		input_subscriber.subscribe_on_scroll_event(Box::new(
			move |scroll: Scroll, _current_state: InputState| {
				if scroll.delta_y.abs() > SCROLL_THRESHOLD {
					...
					let capped_new_distance =
						desired_new_distance.clamp(SCROLL_CLOSEST, SCROLL_FARTHEST);

					let new_position = direction_to_camera * capped_new_distance;

					// ghg_log!("last={:?} new={:?}, direction={:?} dist={}",
					// last_position, new_position, direction_to_camera,
					// capped_new_distance);
