...
use crate::interaction_core::user_inputs::{
    LogicalCursorPosition, MouseButton, SwitchState, UserInput, KeyCodeState, MouseButtonState, Scroll
};
use crate::utils::prelude::*;

pub type KeyState = KeyCodeState<KeyCode>;

// The handlers in this struct are just for lifetime, not really used after register
/// Subscribes to input events from the front end, and provides batches of events which have occurred
/// since the last time it was cleared. Generally, this is used for batching inputs between frames,
/// then handling them all at once. To clear the batch, create a FrameInputHandler, which will clear
...
impl InputBatcher {
    pub fn new(canvas: HtmlCanvasElement) -> Self {
        // TODO: I think RefCell is totally reasonable here, but I need to improve it.
        // It's only handed out to clients that will definitely be run on the same thread, in order.
        // That's how the Javascript event loop works. But that's not exactly extensible, so it should
        // probably use something more robust.
    ...
    pub fn get_current_state(&self) -> InputState {
        self.current_state.borrow().deref().clone()
    }

    // Not for direct use. Use the FrameInputInterpreter to do this at the end of the frame.
...
impl<'a> BatchedInputHandler<'a> {
    pub fn new(input_batcher: &'a mut InputBatcher) -> Self {
        Self {
            input_batcher,
        }
    }
...
impl InputState {
    ...
    #[allow(dead_code)]
    pub fn current_mouse_location(&self) -> Option<LogicalCursorPosition> {
        self.mouse_position
    }
}
...
trait IncorporateState where Self: Clone {
    ...
    fn incorporate(&self, new_input: UserInput<KeyCode>) -> Self {
        ...
        match new_input {
            UserInput::Keyboard(KeyState { key, state: SwitchState::Pressed }) => {
                ...
                if is_new {
                    new_state.unhandled_changes.keyboard_changes.push(KeyState { key, state: SwitchState::Pressed });
                }
            },
            UserInput::Keyboard(KeyState { key, state: SwitchState::Released }) => {
                ...
                if was_removed {
                    new_state.unhandled_changes.keyboard_changes.push(KeyState { key, state: SwitchState::Released });
                }
            },
            UserInput::MouseButton(MouseButtonState { button, state: SwitchState::Pressed }) => {
                ...
                if is_new {
                    new_state.unhandled_changes.mouse_button_changes.push(MouseButtonState { button, state: SwitchState::Pressed });
                }
            },
            UserInput::MouseButton(MouseButtonState { button, state: SwitchState::Released }) => {
                let was_removed = new_state.current_set.remove(&ActiveInput::MouseButton(button.clone()));
                if was_removed {
                    new_state.unhandled_changes.mouse_button_changes.push(MouseButtonState { button, state: SwitchState::Released });
                }
            },
            UserInput::MousePosition(new_position) => {
                ...
                if previous_position.is_some() {
                    new_state.unhandled_changes.mouse_movement = Some(new_position - previous_position.unwrap());
                }
            }
...
fn add_mouse_move_handler(canvas: &HtmlCanvasElement, current_state: Rc<RefCell<InputState>>) -> MouseEventHandler {
    let mouse_move_event_handler = Closure::wrap(Box::new(move |e: MouseEvent| {
        let new_state = UserInput::<KeyCode>::MousePosition(nglm::vec2(
            e.screen_x(),
            e.screen_y(),
        ));
...
fn add_mouse_button_handlers(canvas: &HtmlCanvasElement, current_state: &Rc<RefCell<InputState>>) -> MouseButtonHandlers {
    let mouse_down_event_handler =  make_mouse_button_handler(current_state.clone(), SwitchState::Pressed);
    canvas.set_onmousedown(Some(mouse_down_event_handler.as_ref().unchecked_ref()));

    let mouse_up_event_handler =  make_mouse_button_handler(current_state.clone(), SwitchState::Released);
    canvas.set_onmouseup(Some(mouse_up_event_handler.as_ref().unchecked_ref()));

    MouseButtonHandlers {
        down: mouse_down_event_handler,
        up: mouse_up_event_handler,
    }
}

fn make_mouse_button_handler(current_state: Rc<RefCell<InputState>>, switch_state: SwitchState) -> MouseButtonHandler {
    Closure::wrap(Box::new(move |e: MouseEvent| {
        ...
        let new_state = UserInput::<KeyCode>::MouseButton(MouseButtonState{
            button,
            state: switch_state,
        });
...
fn add_mouse_scroll_handler(canvas: &HtmlCanvasElement, current_state: Rc<RefCell<InputState>>) -> MouseScrollHandler {
...
fn add_keyboard_handlers(canvas: &HtmlCanvasElement, current_state: &Rc<RefCell<InputState>>) -> KeyboardEventHandlers {
    ...
    KeyboardEventHandlers {
        down: key_down_event_handler,
        up: key_up_event_handler,
    }
}

fn make_key_handler(current_state: Rc<RefCell<InputState>>, switch_state: SwitchState) -> KeyboardEventHandler {
    Closure::wrap(Box::new(move |e: KeyboardEvent| {
        if !e.repeat() {
            let new_state = UserInput::<KeyCode>::Keyboard(KeyState{
                key: e.code(),
                state: switch_state,
            });
